...
class Metric():
    def __init__(self, args, predictions, goldens, bert_lengths, sen_lengths, tokens_ranges, ignore_index=-1, sent = None):
        if sent is None:
            self.sent = predictions
        else:
            self.sent = sent
        self.args = args
    ...
    def get_spans(self, tags, length, token_range, type):
        ...
        if start != -1:
            spans.append([start, length - 1])
        return spans
    def get_spans_new(self, tags, length, token_range, type):
        spans = []
        end = -1
    ...
    def find_pair_new(self, tags, aspect_spans, opinion_spans, token_ranges):
        pairs = []
        for al, ar in aspect_spans:
            for pl, pr in opinion_spans:
                if ar == pr: continue
                tag_num = [0] * 6
                for i in range(token_ranges[al][0], token_ranges[ar][0] + 1):
                    for j in range(token_ranges[pr][0], token_ranges[pr][0] + 1):
                        if i < j:
    ...
    def find_triplet_new(self, tags, aspect_spans, opinion_spans, token_ranges):
        triplets = []
        as_set = []
        ap_set = []
        for al, ar in aspect_spans:
            for pl, pr in opinion_spans:
                ...
                tag_num = [0] * 6
                for i in range(token_ranges[al][0], token_ranges[ar][0] + 1):
                    for j in range(token_ranges[pl][0], token_ranges[pl][0] + 1):
                        if i < j:
                            tag_num[int(tags[i][j])] += 1
                        else:
                            tag_num[int(tags[j][i])] += 1
                ...
                if sum(tag_num[3:]) == 0: continue
                sentiment = -1
                if tag_num[3] >= tag_num[5] and tag_num[3] >= tag_num[4]:
                    sentiment = 3
                elif tag_num[5] >= tag_num[3] and tag_num[5] >= tag_num[4]:
                    sentiment = 5
                elif tag_num[4] >= tag_num[5] and tag_num[4] >= tag_num[3]:
                    sentiment = 4
                if sentiment == -1:
                    print('wrong!!!!!!!!!!!!!!!!!!!!')
                    input()
                triplets.append([al, ar, pl, pr, sentiment])
                as_set.append([al, ar, sentiment])
                ap_set.append([al, ar, pl, pr])
        return triplets
    ...
    def score_uniontags(self):
        ...
        predicted_set = set()
        for i in range(self.data_num):
            if self.args.tag_type == 'new':
                golden_aspect_spans = self.get_spans_new(self.goldens[i], self.sen_lengths[i], self.tokens_ranges[i], 1)
                golden_opinion_spans = self.get_spans_new(self.goldens[i], self.sen_lengths[i], self.tokens_ranges[i],
                                                          2)
            else:
                golden_aspect_spans = self.get_spans(self.goldens[i], self.sen_lengths[i], self.tokens_ranges[i], 1)
                golden_opinion_spans = self.get_spans(self.goldens[i], self.sen_lengths[i], self.tokens_ranges[i], 2)
            ...
            golden_opinion_spans = self.get_spans(self.goldens[i], self.sen_lengths[i], self.tokens_ranges[i], 2)
            if self.args.tag_type == 'new':
                golden_tuples = self.find_triplet_new(self.goldens[i], golden_aspect_spans, golden_opinion_spans, self.tokens_ranges[i])
            else:
            ...
            if self.args.tag_type == 'new':
                predicted_aspect_spans = self.get_spans_new(self.predictions[i], self.sen_lengths[i], self.tokens_ranges[i], 1)
                predicted_opinion_spans = self.get_spans_new(self.predictions[i], self.sen_lengths[i], self.tokens_ranges[i], 2)
            else:
                predicted_aspect_spans = self.get_spans(self.predictions[i], self.sen_lengths[i], self.tokens_ranges[i], 1)
                predicted_opinion_spans = self.get_spans(self.predictions[i], self.sen_lengths[i], self.tokens_ranges[i], 2)
            predicted_aspect_spans = self.get_spans(self.predictions[i], self.sen_lengths[i], self.tokens_ranges[i], 1)
            predicted_opinion_spans = self.get_spans(self.predictions[i], self.sen_lengths[i], self.tokens_ranges[i], 2)
            if self.args.tag_type == 'new':
                predicted_tuples = self.find_triplet_new(self.sent[i], predicted_aspect_spans, predicted_opinion_spans, self.tokens_ranges[i])
            else:
